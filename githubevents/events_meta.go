...
type MetaEventHandleFunc func(ctx context.Context, deliveryID string, eventName string, event *github.MetaEvent) error
...
func (g *EventHandler) handleMetaEventAny(ctx context.Context, deliveryID string, eventName string, event *github.MetaEvent) error {
	if event == nil {
...
func (g *EventHandler) MetaEvent(ctx context.Context, deliveryID string, eventName string, event *github.MetaEvent) error {
	...
	err = g.handleMetaEventAny(ctx, deliveryID, eventName, event)
	if err != nil {
		return g.handleError(deliveryID, eventName, event, err)
	}
